...
def init_db():
    ...
    cursor.execute(""" CREATE TABLE IF NOT EXISTS predictions(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER, irradiation REAL, ambient_temperature REAL, module_temperature REAL, hour INTEGER, predicted_power REAL, date TEXT)""")
    cursor.execute("""CREATE TABLE IF NOT EXISTS forecasts (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, location TEXT, date TEXT, predicted_power REAL)""")
    conn.commit()
    conn.close()
init_db()
